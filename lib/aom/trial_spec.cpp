...
Trial_spec::Trial_spec(const YAML::Node& root) {
  ...
  if (root["environment"] && root["environment"]["config_type"]) {
    auto config_type = importer_->pool()->FindMessageTypeByName(root["environment"]["config_type"].as<std::string>());
    if (!config_type) {
  ...
  if (root["trial"] && root["trial"]["config_type"]) {
    auto config_type = importer_->pool()->FindMessageTypeByName(root["trial"]["config_type"].as<std::string>());
    if (!config_type) {
  ...
  for (const auto& a_class : root[cfg_file::actors_key]) {
    ...
    if (a_class["config_type"]) {
      auto config_type = importer_->pool()->FindMessageTypeByName(a_class["config_type"].as<std::string>());
      if (!config_type) {
    ...
    if (root["datalog"] && root["datalog"]["fields"] && root["datalog"]["fields"]["exclude"]) {
    ...
    auto action_space = importer_->pool()->FindMessageTypeByName(a_class["action"]["space"].as<std::string>());
    if (!action_space) {
