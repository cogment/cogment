...
cogment::TrialParams load_params(const YAML::Node& yaml, const Trial_spec& spec) {
  ...
  if (yaml["trial_params"]) {
    ...
    if (yaml_params["trial_config"]) {
      encode_user_config(yaml_params["trial_config"], spec.trial_config_prototype);
    }

    if (yaml_params["environment"]["config"]) {
      encode_user_config(yaml_params["environment"]["config"], spec.env_config_prototype);
    }

    for (auto actor : yaml_params["actors"]) {
      if (actor["config"]) {
        auto class_id = spec.get_class_id(actor["actor_class"].as<std::string>());

        encode_user_config(actor["config"], spec.actor_classes[class_id].config_prototype);
      }
    }
