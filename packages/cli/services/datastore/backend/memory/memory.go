...
func CreateMemoryBackend(maxSamplesSize uint32) (backend.Backend, error) {
	...
	backend := &memoryBackend{
		...
		trialsMutex:           &sync.Mutex{},
		trialIDs:              utils.NewObservableList(),
...
func (b *memoryBackend) evictLruTrials(ctx context.Context) uint32 {
	for {
		...
		go func() {
			...
			frontData.storedSamples = utils.NewObservableList()
...
func (b *memoryBackend) CreateOrUpdateTrials(_ context.Context, trialsParams []*backend.TrialParams) error {
	b.trialsMutex.Lock()
	defer b.trialsMutex.Unlock()
...
func (b *memoryBackend) DeleteTrials(_ context.Context, trialIDs []string) error {
	b.trialsMutex.Lock()
	defer b.trialsMutex.Unlock()
	for _, trialID := range trialIDs {
		if data, exists := b.trials[trialID]; exists {
...
func (b *memoryBackend) GetTrialParams(_ context.Context, trialIDs []string) ([]*backend.TrialParams, error) {
